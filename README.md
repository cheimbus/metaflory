...
- OAuth2.0 카카오 소셜로그인을 이용하여 사용자 로그인 구현 및 유효성검사
  - 유효성검사를 하기위해 카카오 서버에 요청을 계속 보내는것이 비효율적이라고 생각되어, 로그인 시 동시에 서버 자체에서 JWT를 생성하였습니다. 이를 이용하여 유효성 검사를 진행하였습니다.
  - 다른 플랫폼을 적용할 가능성이 있기때문에 (ex google, naver ...)이를 고려하여 코드를 작성하였습니다.
---
...
- OAuth2.0로 로그인을 한 후 유효성 검사를 어떤식으로 진행해야 할 지 고민하였습니다.
  - 먼저 카카오 소셜로그인을 하면 카카오 accesstoken과 refreshtoken을 받아옵니다.
