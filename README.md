...
## 9. 어려웠던 부분 해결과정
- 관리자가 이미지를 등록하고 해당 이미지를 서버에서 정적 파일로 서비스하는 데 어려움을 느꼈습니다.
  - nest.js에서 form-data형식으로 파일을 받기 위해서 @UseInterceptors 데코레이터를 사용하였습니다.
  - 여기서 nest.js는 express 기반이므로 multer 미들웨어를 지원하기 때문에 이를 이용하여 form-data로 이미지 형식을 받아왔습니다.
  - 받은 이미지는 원하는 폴더에 저장되었습니다.
  - nest.js에서 정적 파일로 서비스하기 위한 패키지인 ServeStaticModule를 사용하였습니다.
  - 여기서 문제였던 것은, npm run start:dev 즉, nest.js에서 지원하는 hot reloading할 때와 npm run start 즉, 빌드된 dist폴더를 실행하였을 때, 정적 파일을 서비스하는데서 경로가 서로 달랐습니다.
  - 실제 서비스를 할 때 npm run start를 하기 때문에 해당 경로를 확인하였습니다. 따라서 해당 모듈의 forRoot가 어떤식으로 실행이되고, 어떤 경로로 정적 파일을 서비스하는지 console.log로 확인하여 해결 하였습니다.
---
- OAuth2.0로 로그인을 한 후 유효성 검사를 어떤식으로 진행해야 할 지 고민하였습니다.
  - 먼저 카카오 소셜로그인을 하면 카카오 전용 accesstoken과 refreshtoken을 받아옵니다.
  - 이 카카오에서 받아온 accesstoken을 이용해서 유효성검사를 하게된다면, 토큰이 만료가 될때마다 카카오 서버에 refresh 요청을 보내야 한다는 것이 매우 비효율적이라고 생각이 들었습니다.
  - 따라서 소셜로그인을 한 동시에 서버 자체에서 JWT를 발행해서 제공하는 것이 낫겠다 생각을 하여, 서버 accesstoken, refreshtoken을 각각 발행하여 유효성검사를 하게되었습니다.
